...
public class ReceivePortGroup<M> implements Mix<M> {
    ...
    private static final Mode modeDefault = Mode.NORMAL;
    private static final boolean soloDefault = false;
    private static final SoloEffect soloEffectDefault = SoloEffect.PAUSE_OTHERS;
    private static final boolean alwaysOpenDefault = false;

    private static final Channel<? super Object> changedCh = Channels.newChannel(1, Channels.OverflowPolicy.DISPLACE, false, true);
    ...
    public ReceivePortGroup(final boolean alwaysOpen) {
        this(ImmutableList.<ReceivePort<? super M>>of(), alwaysOpen);
    }

    public ReceivePortGroup() {
        this(ImmutableList.<ReceivePort<? super M>>of(), alwaysOpenDefault);
    }
